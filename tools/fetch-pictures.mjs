...
/* The same list as a script, in list order, for the page to load the way it
   loads animals.js — a page opened from disk cannot fetch a JSON file. */
const rows = list.filter(it => out[it.name]).map(it => {
  const c = out[it.name];
  return { name: it.name, image: "pictures/" + c.image, author: c.author, licence: c.licence, source: c.source };
});
fs.writeFileSync(path.join(DIR, "pictures.js"),
  ...
  "(function (root) {\n  var PICTURES = " + JSON.stringify(rows, null, 1).replace(/\n/g, "\n  ") + ";\n" +
